...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", async function (event) {
        ...
        if (!email || !/^\S+@\S+\.\S+$/.test(email)) return alert("Informe um email válido.");
        if (telefone.length !== 9) return alert("Telefone deve ter exatamente 9 números.");
        if (!classeSelecionada) return alert("Selecione a classe.");
        if (!turma) return alert("Informe a turma.");
        if (!dataNascimento) return alert("Informe a data de nascimento.");
        if (dataNascimento > hoje) return alert("Data de nascimento não pode ser maior que hoje.");
        if (["10ª Classe", "11ª Classe", "12ª Classe", "13ª Classe"].includes(classe) && curso === "") {
            document.getElementById('mensagem').textContent = `Por favor, selecione o curso para a ${classe}.`;
            document.getElementById('mensagem').style.color = "red";
            return; // Impede envio
        }
        const formData = new FormData(this);
        const dados = Object.fromEntries(formData.entries());
        const classe = parseInt(dados.classe);
    if (["10ª Classe", "11ª Classe", "12ª Classe", "13ª Classe"].includes(classe) && (!dados.curso || dados.curso.trim() === "")) {
    document.getElementById('mensagem').textContent = "Por favor, selecione o curso para a classe " + classe + ".";
    document.getElementById('mensagem').style.color = "red";
