...
Perfeitíssimo, meu bem! Vou refatorar pra ti, limpinho, sem os `El`, organizadinho e com a validação visual dos campos obrigatórios destacando em vermelho os que estão vazios — e respeitando as regras que você disse:

* **Se clicou em "Guardar"** → valida só `tipoDocumento` e `titulo`.
* **Se clicou em "Criar Documento"** → valida todos os campos obrigatórios.
* Exibe mensagens no `small` abaixo dos inputs com `data-erro-id`.
* Remove o `El` no nome das variáveis.
* Se o campo estiver preenchido, tira o erro.

Olha só o código ajustado, meu amor:
...
* Removi os `El` nas variáveis.
* Separei tudo em:

  * `coletarDadosContrato()` — coleta os dados.
  * `validarCampos(dados, tipo)` — valida os campos conforme o botão.
  * `exibirErro()` — coloca borda vermelha e mostra texto.
  * `limparErros()` — limpa antes de validar de novo.
  * Funções separadas de `enviarContrato` e `guardarContrato`.
* Eventos nos dois botões funcionando conforme o solicitado.

---

Se quiser, posso montar um exemplo HTML junto já com esse JS linkado pra testar de boa ✨ Quer?
