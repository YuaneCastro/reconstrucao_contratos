...
document.getElementById('form-estudante').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (response.ok) {
            const encarregado = await response.json();

            // Verifica se o encarregado existe e se os dados correspondem
            if (encarregado.nome === nome && encarregado.telefone === telefone) {
                // Tudo certo, pode cadastrar
                aviso.textContent = '';
                e.target.submit(); // envia o formulário
            } else {
                aviso.style.color = 'red';
                aviso.textContent = 'Verifique as informações do encarregado já cadastrado.';
                botao.disabled = false; // O botão continua ativo para tentar corrigir
            }
        } else {
            // Não existe encarregado → pode prosseguir
            aviso.textContent = '';
            e.target.submit();
        }
    } catch (error) {
        console.error(error);
        aviso.style.color = 'red';
        aviso.textContent = 'Erro ao verificar encarregado.';
    }
});
