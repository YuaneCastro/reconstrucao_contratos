const tsClasses = new TomSelect('#classes', { plugins: ['remove_button'] });
const tsCursos = new TomSelect('#cursos', { plugins: ['remove_button'] });
const tsTurmas = new TomSelect('#turmas', { plugins: ['remove_button'] });

const enviarTodosCheckbox = document.getElementById('enviarTodos');
const filtros = document.getElementById('filtros');
const cursoContainer = document.getElementById('cursoContainer');

// Mostrar cursos apenas se 10, 11 ou 12 forem selecionadas
function atualizarCursos() {
  const classesSelecionadas = tsClasses.getValue();
  const mostrar = classesSelecionadas.includes('10') || classesSelecionadas.includes('11') || classesSelecionadas.includes('12') || classesSelecionadas.includes('13');
  cursoContainer.classList.toggle('hidden', !mostrar);
}

// Mostrar ou esconder filtros conforme checkbox
enviarTodosCheckbox.addEventListener('change', () => {
  filtros.style.display = enviarTodosCheckbox.checked ? 'none' : 'block';
});

// Atualizar visibilidade dos cursos quando classes forem alteradas
tsClasses.on('change', atualizarCursos);

// Atualiza no carregamento inicial também
atualizarCursos();
