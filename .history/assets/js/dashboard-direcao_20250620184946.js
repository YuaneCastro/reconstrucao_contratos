...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const dataNascimentoValor = dataNascimentoInput.value.trim();
...
const dataNascimento = new Date(dataNascimentoValor);
dataNascimento.setHours(0, 0, 0, 0);

if (dataNascimento > hoje) {
  document.getElementById('mensagem').textContent = "A data de nascimento não pode ser no futuro.";
  document.getElementById('mensagem').style.color = "red";
  return; // Impede envio
}
...
function validarCampos(dados, tipo) {
    ...
    if (tipo === 'enviar') {
        ...
        if (dataExpiracao.value.trim() === '') {
            ...
            const dataSelecionada = new Date(dataExpiracao.value);
            dataSelecionada.setHours(0,0,0,0);

            if (dataSelecionada < hoje) {
                exibirErro(dataExpiracao, "A data não pode ser anterior a hoje.");
                valido = false;
            }
        }
