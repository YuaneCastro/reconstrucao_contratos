...
function exibirErro(campo, mensagem) {
  ...
  document.getElementById('contratoForm').addEventListener('submit', async function(event) {
    ...
    let valido = true;
     ...
     // Validação específica para 'Guardar'
     if (botaoClicado === 'botao_guardar') {
        if (!tipoDocumentoEl.value.trim()) {
            mostrarErro(tipoDocumentoEl, "Informe o tipo de documento.");
            valido = false;
        }
        if (!tituloEl.value.trim()) {
            mostrarErro(tituloEl, "Informe o título do documento.");
            valido = false;
        }
    }
    // Validação completa para 'Criar Documento'
    if (botaoClicado === 'botaoSubmit') {
        if (!tipoDocumentoEl.value.trim()) {
            mostrarErro(tipoDocumentoEl, "Informe o tipo de documento.");
            valido = false;
        }
        ...
        if (!conteudoEl.value.trim()) {
            mostrarErro(conteudoEl, "Informe o conteúdo do documento.");
            valido = false;
        }
        if (!dataExpiracaoEl.value) {
            mostrarErro(dataExpiracaoEl, "A data de expiração é obrigatória.");
            valido = false;
        }
