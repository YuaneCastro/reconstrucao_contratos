...
editdocumento.forEach(button => {
    ...
    if (dataExpiracao) {
        const dataUtc = new Date(dataExpiracao);
const dataLocal = new Date(dataUtc.getTime() + (dataUtc.getTimezoneOffset() * 60000));
document.getElementById("dataExpiracao").value = dataLocal.toISOString().split('T')[0];
      } else {
...
function formatarDataLocal(dataUtc) {
    ...
    // Ajusta o fuso local e pega a data ISO no formato certo
