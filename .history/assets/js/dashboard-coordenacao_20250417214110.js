...
function atualizarTituloFormulario() {
    ...
    const titulo = tipo === "contrato" ? "Criar Novo Contrato" : "Criar Novo Comunicado";
    const botaoTexto = tipo === "contrato" ? "Criar Contrato" : "Criar Comunicado";
    document.getElementById("formTitle").innerText = titulo;
    document.getElementById("botaoSubmit").innerText = botaoTexto;
  }
  ...
  function verificarCurso(select, id) {
    ...
    if (value === "10ª Classe" || value === "11ª Classe" || value === "12ª Classe" || value === "12ª Classe") {
  ...
  document.getElementById('contratoForm').addEventListener('submit', async function(event) {
    ...
    if(resposta) {
        ...
        document.querySelectorAll('.especificacao').forEach((div) => {
            ...
            especificacoes.push({
                classe,
                curso: (["10", "11", "12"].includes(classe)) ? curso : null,
                turma
            });
        });
