...
function validarParaEnvio(dados) {
    if (!dados.tipoDocumento || !dados.titulo || !dados.conteudo) {
        alert("Preencha todos os campos obrigatórios.");
        return false;
    }

    if (!dados.enviarParaTodos && dados.especificacoes.length === 0) {
        alert("Você deve selecionar pelo menos uma raiz ou marcar a opção de enviar para todos.");
        return false;
    }

    if (!dados.dataExpiracao) {
        alert("A data de expiração é obrigatória para envio.");
        return false;
    }
...
document.getElementById('botaoSubmit').addEventListener('click', async function(event) {
    ...
    if (validarParaEnvio(dados)) {
        const resposta = confirm("Deseja realmente enviar o contrato aos encarregados agora?\n\nSe ainda estiver em dúvida, clique em 'Cancelar' e use a opção 'Guardar' para salvar como rascunho.");
        if (resposta) {
...
function validarParaGuardar(dados) {
    if (!dados.tipoDocumento || !dados.titulo) {
        alert("Preencha pelo menos o tipo de documento e o título.");
        return false;
    }
...
document.querySelectorAll('#contratoForm [required]').forEach(campo => {
    campo.addEventListener('input', function() {
      const erroId = this.getAttribute('data-erro-id');
  ...
  // Valida também ao sair do campo (blur)
  document.querySelectorAll('#contratoForm [required]').forEach(campo => {
