...
function handleViewContract(button) {
  ...
  const id = button.getAttribute('data-id_encarregado');
  const id = button.getAttribute('data-id_estudante');
...
async function handleAssinarContrato(id) {
  ...
  try {
    ...
    if (data.sucesso) {
        ...
        alert("Erro ao assinar o contrato.");
    }
  } catch (err) {
