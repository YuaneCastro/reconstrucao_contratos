...
const nome = document.getElementById('encarregado_nome');
...
async function verificarInformacoes() {
    ...
    if (nomeValor && emailValor && telefoneValor) {
        try {
            const response = await fetch('/cadastro', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    encarregado_nome: nomeValor,
                    encarregado_email: emailValor,
            ...
            if (data.sucesso === false) {
                ...
                aviso.textContent = 'Informações válidas. Pode cadastrar.';
...
nome.addEventListener('input', verificarInformacoes);
email.addEventListener('input', verificarInformacoes);
telefone.addEventListener('input', verificarInformacoes);