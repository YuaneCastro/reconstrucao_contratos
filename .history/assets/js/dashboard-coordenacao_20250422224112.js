...
document.addEventListener("DOMContentLoaded", function () {
    ...
    classeSelectCadastro?.addEventListener("change", function () {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    editGuardianButtons.forEach(button => {
        button.addEventListener("click", () => {
            ...
            alert(document.getElementById("curso").value = estudante.curso || "")
            exibirEstudantesAssociados(estudantes);

            if (estudantes.length > 0) {
                const estudante = estudantes[0]; // pegando o primeiro associado
                document.getElementById("classe").value = estudante.classe;
                document.getElementById("curso").value = estudante.curso || "";
                atualizarVisibilidadeCurso_atualizar(estudante.classe, estudante.curso);
            } else {
                atualizarVisibilidadeCurso_atualizar(""); // esconde se não tiver estudante
            }
    ...
    function mostrarCursoSeNecessario(estudante) {
        const container = document.getElementById("cursoContainer");
        const campoCurso = document.getElementById("curso");
    
        // Classes que exigem curso
        const classesComCurso = ["10ª Classe", "11ª Classe", "12ª Classe", "13ª Classe"];
    
        if (classesComCurso.includes(estudante.classe)) {
            container.style.display = "block";
            campoCurso.value = estudante.curso || "";
        } else {
            container.style.display = "none";
            campoCurso.value = "";
        }
    }
    function atualizarVisibilidadeCurso_atualizar(classeSelecionada, cursoSelecionado = ""){
        const container = document.getElementById("cursoContainer");
        const campoCurso = document.getElementById("curso");
        // Classes que possuem curso
        const classesComCurso = ["10ª Classe", "11ª Classe", "12ª Classe", "13ª Classe"];
    
        if (classesComCurso.includes(classeSelecionada)) {
            container.style.display = "block";
            // Se veio um curso pré-definido, coloca no campo
            if (cursoSelecionado) {
                campoCurso.value = cursoSelecionado;
            } else {
                campoCurso.value = ""; // limpa se não vier nada
            }
        } else {
            container.style.display = "none";
            campoCurso.value = ""; // limpa o valor se não for classe com curso
        }
    }
    const classeSelectEdicao = document.getElementById("classe");
    classeSelectEdicao?.addEventListener("change", function () {
        atualizarVisibilidadeCurso_atualizar(this.value);
    });
