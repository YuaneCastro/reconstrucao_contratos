...
document.addEventListener('DOMContentLoaded', () => {
  ...
  cards.forEach(card => {
    card.addEventListener('click', () => {
      ...
      const dataEmissao = card.getAttribute('data-emissao');
      const dataExpiracao = card.getAttribute('data-expiracao');
      ...
      // Adicionar botão se for contrato
      if (tipo.toLowerCase() === 'contrato') {
        html += `
          <div style="text-align: center; margin-top: 30px;">
            <button class="btn-assinar" onclick="handleAssinarContrato('${id}')">
      ...
      // Atualiza o conteúdo e exibe a seção de visualização
      conteudoContrato.innerHTML = html;
      showSection('visualizacaoContrato');
    });
  });
