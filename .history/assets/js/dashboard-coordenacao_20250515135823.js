...
editdocumento.forEach(button => {
    ...
    if (dataExpiracao && dataExpiracao !== "Sem data") {
        document.getElementById("dataExpiracao").value = new Date(dataExpiracao).toISOString().split("T")[0];
        document.getElementById("dataExpiracaoGroup").style.display = "block";
      } else {
        document.getElementById("dataExpiracao").value = "";
        document.getElementById("dataExpiracaoGroup").style.display = "none";
      }

      document.getElementById("enviarTodos").checked = (enviarParaTodos === "true" || enviarParaTodos === "1");
    });

    const containerEspecificacoes = document.getElementById("especificacoesContainer");
    ...
    if (categorias) {
        ...
        categoriasArray.forEach(cat => {
          ...
          // Pega a última especificação adicionada
          const ultimaEspecificacao = containerEspecificacoes.lastElementChild;
          const classeSelect = ultimaEspecificacao.querySelector(`select[name^="classe_"]`);
          const cursoContainerId = ultimaEspecificacao.querySelector('div[id^="cursoContainer-"]').id;
          const cursoSelect = ultimaEspecificacao.querySelector(`select[name^="curso_"]`);
          const turmaSelect = ultimaEspecificacao.querySelector(`select[name^="turma_"]`);
    
          // Define os valores
          classeSelect.value = cat.classe;
          turmaSelect.value = cat.turma;
    
          // Se a classe for superior a 9ª, mostra o container de curso
          if (["10ª Classe", "11ª Classe", "12ª Classe", "13ª Classe"].includes(cat.classe)) {
            document.getElementById(cursoContainerId).classList.remove("hidden");
            cursoSelect.value = cat.curso;
          }
        });
      }
});