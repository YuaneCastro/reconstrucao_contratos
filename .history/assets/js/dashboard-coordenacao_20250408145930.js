...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const formEncarregado = document.getElementById("form-editar");

    formEncarregado.addEventListener("submit", async (event) => {
        event.preventDefault();  // Impede o envio tradicional do formulário
    
        const validarTelefone = (telefone) => {
            const regex = /^[0-9]{9}$/; // Aceita números de 10 a 15 dígitos
            return regex.test(telefone);
        };
    
        // Coletar os dados do formulário
        const formData = new FormData(formEncarregado);
        const data = new URLSearchParams(formData);
    
        // Validar o telefone antes de enviar
        const telefone = formEncarregado['telefone'].value; // Captura o valor do telefone
        if (!validarTelefone(telefone)) {
            alert('Telefone inválido! Deve ter 9 dígitos.');
            return; // Não envia o formulário se o telefone for inválido
        }
    
        console.log("Formulário enviado!"); // Verifique se o evento está sendo capturado
        console.log('Dados do Formulário:', data.toString());
    
        try {
            // Enviar dados via fetch
            const response = await fetch(formEncarregado.action, {
                method: formEncarregado.method,
                body: data
            });
            ...
            if (!result.sucesso) {
                ...
                // Recarregar a página ou realizar outra ação após sucesso
                location.reload();
            }
        } catch (error) {
            console.error('Erro ao tentar atualizar os dados do encarregado:', error);
            alert('Houve um erro ao tentar atualizar os dados do encarregado.');
        }
    });
