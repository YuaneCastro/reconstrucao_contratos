document.addEventListener('DOMContentLoaded', () => {
    if (userRole === 'secretaria') {
      // Esconde as divs proibidas
      ['logs', 'novo_contrato', 'dashboard'].forEach(id => {
        const el = document.getElementById(id);
        if (el) el.remove(); // Remove completamente do DOM
      });
  
      // Remove os menus ou parágrafos clicáveis relacionados
      document.querySelectorAll('[onclick*="showSection(\'logs\')"]').forEach(el => el.remove());
      document.querySelectorAll('[onclick*="showSection(\'dashboard\')"]').forEach(el => el.remove());
  
      // Ativa a div exclusiva para a secretaria
      const secDiv = document.getElementById('secretaria_access');
      if (secDiv) {
...
document.getElementById('enviarTodos').addEventListener('change', function () {
  ...
  function limparErros() {
    document.querySelectorAll('.erro-mensagem').forEach(e => e.textContent = '');
    document.querySelectorAll('.form-group select, .form-group input, .form-group textarea').forEach(e => {
      e.classList.remove('erro');
      e.style.borderColor = '#ccc'; // Cor neutra
    });
  }
function exibirErro(campo, mensagem) {
    const erroId = campo.getAttribute('data-erro-id');
    if (erroId) {
...
function validarCampos(dados, tipo) {
    ...
    if (tipoDocumento.value.trim() === '') {
        exibirErro(tipoDocumento, "Campo obrigatório.");
        valido = false;
    }

    if (titulo.value.trim() === '') {
        exibirErro(titulo, "Campo obrigatório.");
        valido = false;
    }

    if (tipo === 'enviar') {
        if (conteudo.value.trim() === '') {
            exibirErro(conteudo, "Campo obrigatório.");
            valido = false;
        }

        if (dataExpiracao.value.trim() === '') {
            exibirErro(dataExpiracao, "Campo obrigatório.");
            valido = false;
        }

        if (!dados.enviarTodos && dados.especificacoes.length === 0) {
...
async function guardarContrato(dados) {
    try {
        ...
        if (data.sucesso) {
            ...
            alert("Infelizmente não pode executar determinada ação, pois não pertence aos administradores. ");
        }
    } catch (err) {
...
document.getElementById('botao_guardar').addEventListener('click', async function(event) {
    event.preventDefault();
    const dados = coletarDadosContrato();
    if (validarCampos(dados, 'guardar')) {
        await guardarContrato(dados);
    }
});
...
document.getElementById('botaoSubmit').addEventListener('click', async function(event) {
    event.preventDefault();
    const dados = coletarDadosContrato();
    if (validarCampos(dados, 'enviar')) {
        const resposta = confirm("Deseja realmente enviar o contrato aos encarregados agora?");
        if (resposta) {
            await enviarContrato(dados);
        }
    }
});
...
//-------encarregados associados----------
async function mostrarAssociados(elemento){
    const contratoId = elemento.getAttribute('data-id'); 
    ...
    try {
        ...
        const data = await response.json();
        if (data.sucesso) {
            const tbody = document.getElementById('associados-body');
            tbody.innerHTML = '';
            // Preenche os encarregados na tabela
            data.encarregados.forEach(encarregado => {
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td>${encarregado.id}</td>
                    <td>${encarregado.encarregado_id}</td>
                    <td>${encarregado.estudante_id}</td>
                    <td>${encarregado.ip_assinatura}</td>
                    <td>${encarregado.data_assinatura}</td>
                    <td>${encarregado.estado}</td>
                `;
                tbody.appendChild(tr);
            });
            // Exibe a seção de encarregados
            showSection('associados'); 
        } else {
...
function abrirNovoDocumento(){
    limparFormulario();
    showSection('novo_contrato');
}

const editdocumento = document.querySelectorAll(".editar_documento");
editdocumento.forEach(button => {
    button.addEventListener("click", () => {
        limparFormulario(); // limpa só as especificações e checkbox antes
    const id = button.getAttribute("data-id");
    const tipo = button.getAttribute("data-tipo");
    const titulo = button.getAttribute("data-titulo");
    const descricao = button.getAttribute("data-descricao");
    const dataExpiracao = button.getAttribute("data-data_expiracao");
    const categoriasJson = button.getAttribute("data-categorias");
    const enviarParaTodos = button.getAttribute("data-enviar_para_todos");
    ...
    if (dataExpiracao) {
        document.getElementById("dataExpiracao").value = dataExpiracao; // onde dataExpiracao = "2025-05-22"

      } else {
        document.getElementById("dataExpiracao").value = "";
      }
      // Sempre mostra o grupo, não esconde nunca
      document.getElementById("dataExpiracaoGroup").style.display = "block";     


      document.getElementById("enviarTodos").checked = (enviarParaTodos === "true" || enviarParaTodos === "1");

      if (categorias && categorias.length > 0) {
        categorias.forEach(cat => {
          adicionarEspecificacaoComDados(cat);
        });
      }      
    });
});
function adicionarEspecificacaoComDados(categoria) {
    const container = document.getElementById('especificacoesContainer');
    const div = document.createElement('div');
    div.classList.add('especificacao');
    div.setAttribute('data-id', contador);
    ...
    // Preencher os selects com os valores de categoria
    const classeSelect = div.querySelector(`select[name="classe_${contador}"]`);
    const cursoSelect = div.querySelector(`select[name="curso_${contador}"]`);
    const turmaSelect = div.querySelector(`select[name="turma_${contador}"]`);
  
    if (categoria.classe) classeSelect.value = categoria.classe;
    if (categoria.curso) {
      cursoSelect.value = categoria.curso;
      document.getElementById(`cursoContainer-${contador}`).classList.remove('hidden');
    }
    if (categoria.turma) turmaSelect.value = categoria.turma;
...
function limparFormulario() {
    ...
    form.querySelectorAll("input[type='text'], input[type='hidden'], input[type='date']").forEach(input => {
      input.value = '';
    });
    ...
    // Limpa container de especificações
