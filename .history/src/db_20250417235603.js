...
const guardar_documento_e_enviar = async (tipo, titulo, descricao, especificacoes, enviarParaTodos) => {
    try {
        ...
        if (enviarParaTodos) {
            // 2. Buscar TODOS os encarregados únicos da tabela de estudantes
            const encResult = await pool.query(`SELECT id FROM encarregados`);
            encarregados = encResult.rows;
            
            // Insere entrada genérica em documentos_categorias
            await pool.query(
            ...
            // Agora buscar os encarregados com base nas especificações
            const queries = [];
            for (const espec of especificacoes) {
                queries.push(`
                    SELECT DISTINCT e.id FROM encarregados e
                    JOIN estudantes s ON e.id = s.encarregado_id
                    WHERE s.classe = '${espec.classe}'
                    ${espec.curso ? `AND s.curso = '${espec.curso}'` : ''}
                    ${espec.turma !== 'todos' ? `AND s.turma = '${espec.turma}'` : ''}
                `);
            }

            const uniqueEncarregados = new Set();
            for (const q of queries) {
                const res = await pool.query(q);
                res.rows.forEach(r => uniqueEncarregados.add(r.id));
            }

            encarregados = Array.from(uniqueEncarregados).map(id => ({ id }));
        }

        // Inserir as associações na tabela de assinaturas
