    ...
    try {
        const result = await pool.query(
            `INSERT INTO logins_coordenacao (email, codigo_otp, ip, usado, data_envio, data_login) 
             ...
             RETURNING id`,
            [email, bcrypt_code, ip_user, false, new Date()]
        );
...
const atualizar_login_coordenacao = async (id) => {
    if (!id) {
    ...
    try {
        await pool.query(
            "UPDATE logins_coordenacao SET usado = true, data_login = NOW() WHERE id = $1",
            [id]
        );
    } catch (error) {
...
const buscar_email = async (email) =>{
    const result = await pool.query('SELECT * FROM encarregados WHERE  email = $1', [email]);
    return result.rows[]
}
const buscarEncarregadoPorTelefone = async (telefone) => {
    const result = await pool.query('SELECT * FROM encarregados WHERE telefone = $1',[telefone]);
    return result.rows[0];
}
const buscarEncarregadoPorNome = async(nome) => {
    const result = await pool.query('SELECT *FRROM encarregados WHERE nome = $1', [nome]);
    return result.rows[0];
}
...
module.exports = {
    ...
    buscarEncarregadoPorTelefone,
    buscarEncarregadoPorNome,
    criarEncarregado,
