...
const createCode = async(email,confcode) => {
    const rawCode = Math.floor(100000 + Math.random() * 900000).toString(); // Código de 6 dígitos
    const hashedCode = await bcrypt.hash(rawCode, 10); // Criptografar código
...
const findCode  = async (email, inputCode) => {
    ...
    try {
        ...
        if (rows.length === 0 || !rows[0].codigo_verificacao) return false;

        const isMatch = await bcrypt.compare(inputCode, rows[0].codigo_verificacao);
        return isMatch;
    } catch (error) {
