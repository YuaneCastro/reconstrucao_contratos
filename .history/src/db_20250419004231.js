...
const buscarDocumentosDoEncarregado = async (id) => {
    const result = await pool.query(`
        SELECT d.id, d.tipo, d.titulo, d.descricao, d.data_emissao
        FROM documentos d
...
const guardar_documento_e_enviar = async (tipo, titulo, descricao, especificacoes, enviarParaTodos, dataExpiracao) => {
    try {
            // Define se deve incluir data_expiracao
            const insertQuery = tipo === 'contrato'
                ?
        ...
        VALUES ($1, $2, $3, $4)
        RETURNING id`,[tipo, titulo, descricao, dataExpiracao]);
