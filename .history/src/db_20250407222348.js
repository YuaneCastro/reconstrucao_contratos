...
const verifica_estudante = async (nome, id) => {
    const result = await query.pool.query('SELECT * FROM estudantes WHERE nome = $1 AND id != $2', [nome, id]);
    return result.rows;
};
const verifica_encarregado = async(nome, email, telefone, id) => {
    const result = await query.pool(`SELECT * FROM encarregados WHERE (nome = $1 OR email = $2 OR telefone = $3) AND id != $4`,
    [nome, email, telefone, id]);
    return result.rows;
}
