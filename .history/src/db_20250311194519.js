...
const addUserToDB = async (nome, email, senha) => {
    ...
    try {
        ...
        await logEvent(userId, null, "Cadastro", `Usuário ${nome} (${email}) foi cadastrado.`);
...
const updateUser = async (id, nome, email, senhaHash) => {
    try {
        ...
        await logEvent(id, null, "Atualização", `Usuário ID ${id} atualizou suas informações.`);
...
const logEvent = async (usuario_id, documento_id = null, tipo_evento, descricao) => {
    const query = `
        INSERT INTO historico_eventos (usuario_id, documento_id, tipo_evento, descricao)
        VALUES ($1, $2, $3, $4)
    `;
    const values = [usuario_id, documento_id, tipo_evento, descricao];
