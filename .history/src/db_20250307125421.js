...
const updateUser = async (nome, email, senhaHash) => {
    try {
        ...
        if (email && email !== existingUser.email && (await checkIfEmailExists(email, id))) {
            throw new Error('O e-mail já está em uso');
        }

        // Verifica se o novo nome já está em uso por outro usuário
        if (nome && nome !== existingUser.nome && (await checkIfUsernameExists(nome, id))) {
