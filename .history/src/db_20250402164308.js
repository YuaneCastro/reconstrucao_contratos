...
const enviar_codigo = async (email, bcrypt_code, ip_user, role) => {
    try {
        const result = await pool.query(
            `INSERT INTO logins_coordenacao (email, codigo_otp, ip, usado, data_envio, role, data_login) 
             VALUES ($1, $2, $3, $4, $5, $6, DEFAULT) 
             RETURNING id`,
            [email, bcrypt_code, ip_user, false,  role, new Date()]
        );
