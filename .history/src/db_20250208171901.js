...
const createOTP = async(userId, code, expirationTime) => {
     const query = 'INSERT INTO otps (user_id, otp_code, expires_at) VALUES ($1, $2, $3)RETURNING *;';
     const values = [userId, code, expirationTime];
     try {
        const { rows } = await pool.query(query, values);
        console.log("OTP criado com sucesso:", rows[0]);
        return rows[0];
    } catch (error) {
