...
const findCode = async (email, inputCode) => {
    ...
    try {
        ...
        if (!rows[0].codigo_verificacao) {
            console.log(`Nenhum código de verificação encontrado para o email: ${email}`);
            return false;
        }

        const savedCode = rows[0].codigo_verificacao.trim(); // Remover espaços extras
        console.log(`Código salvo no banco: "${savedCode}", Código digitado: "${inputCode.trim()}"`);

        // Comparação correta usando bcrypt
        const isMatch = bcrypt.compareSync(inputCode.trim(), savedCode);
        return isMatch;
