...
const atualizarTodosEncarregados = async () => {
    try {
        const query = `
            UPDATE encarregados
            SET atualizado_em = NOW(), outra_coluna = NOW()
            RETURNING *;
        `;

        const resultado = await pool.query(query);

        // Retorna os novos valores atualizados
        return resultado.rows;

    } catch (error) {
        console.error("Erro ao atualizar todos os encarregados:", error);
        throw error;
    }
};
