...
const enviar_codigo = async (email, bcrypt_code, ip_user) => {
    try {
        const result = await pool.query( // ✅ Agora result está definido
...
const buscar_codigoUTP = async(id, email) => {
    try {
        console.log("🔍 Buscando código OTP para ID:", id, "e Email:", email);
        ...
        return result.rows.length > 0 ? result.rows[0].codigo_otp : null;
    } catch (error) {
        console.error("❌ Erro ao buscar código OTP:", error);
        return null;
    }
};
const atualizar_login_cordenacao = async(id) =>{
    if (!id) {
