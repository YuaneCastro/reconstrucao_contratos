...
const buscarDocumentosDoEncarregado = async (id) => {
    const result = await pool.query(`
        SELECT d.id, d.tipo, d.titulo, d.descricao, d.data_emissao, d.data_expiracao 
        FROM documentos d
...
async function buscarContratosPendentes(encarregadoId) {
    const query = `
      SELECT d.id, d.titulo, d.descricao, d.data_emissao, d.data_expiracao
      FROM documentos d
      JOIN assinaturas_documento ad ON d.id = ad.documento_id
      WHERE ad.encarregado_id = $1
        AND d.tipo = 'contrato'
        AND ad.estado = 'pendente'
    `;
  
    const result = await pool.query(query, [encarregadoId]);
    return result.rows;
  }
...
const guardar_documento_e_enviar = async (tipo, titulo, descricao, especificacoes, enviarParaTodos, dataExpiracao) => {
    try {        
         const result = await pool.query( `INSERT INTO documentos (tipo, titulo, descricao, data_expiracao)
        VALUES ($1, $2, $3, $4) RETURNING id`,[tipo, titulo, descricao, dataExpiracao]);
