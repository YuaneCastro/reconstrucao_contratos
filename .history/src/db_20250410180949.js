...
const eliminar_log = async (client, id) => {
    const query = 'DELETE FROM log_atividades WHERE encarregado_id = $1';
    await client.query(query, [id]);
};

const eliminar_dos_tokens = async (client, id) => {
    const query = 'DELETE FROM tokens_redefinicao WHERE encarregado_id = $1';
    await client.query(query, [id]);
};

const eliminar_estudantes = async (client, id) => {
    const query = 'DELETE FROM estudantes WHERE encarregado_id = $1';
    await client.query(query, [id]);
};

const eliminar_encarregado = async (client, id) => {
    const query = 'DELETE FROM encarregados WHERE id = $1';
    await client.query(query, [id]);
};

const eliminar_tudo = async (id) => {
    const client = await pool.connect();  // Obtém o cliente do pool para iniciar a transação

    try {
        await client.query('BEGIN'); // Inicia a transação

        // Chama as funções de deleção dentro da transação usando o mesmo cliente
        await eliminar_log(client, id);
        await eliminar_dos_tokens(client, id);
        await eliminar_estudantes(client, id);
        await eliminar_encarregado(client, id);
        ...
        await client.query('ROLLBACK'); // Se ocorrer erro, desfaz todas as mudanças
