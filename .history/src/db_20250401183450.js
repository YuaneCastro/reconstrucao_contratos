...
const lista_coordenacao = async () => {
    try{
        const result = await pool.query(`SELECT * FROM logins_coordenacao ORDER BY id DESC;`);
        const logsFormatados = result.rows.map(log => ({
            ...log,
            data_envio: log.data_envio ? log.data_envio.toLocaleString('pt-BR') : "Sem registro",
            data_login: log.data_login ? log.data_login.toLocaleString('pt-BR') : "Sem registro"
        }));
    }catch(error){
