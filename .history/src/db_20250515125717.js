...
const guardar_documento = async(tipo, titulo, descricao = null, especificacoes = [],  enviarParaTodos = false, dataExpiracao = null, id=null) => {
    try {
        ...
        if (enviarParaTodos) {
            ...
            // Se 'enviarParaTodos' for false, atualiza as especificações
            // Primeiro, apaga todas as especificações antigas
            await pool.query(`
                DELETE FROM documentos_categorias WHERE documento_id = $1
            `, [documentoId]);
            ...
            for (const espec of especificacoes) {
                ...
                await pool.query(`
                    INSERT INTO documentos_categorias (documento_id, curso, classe, turma, enviar_para_todos)
                    VALUES ($1, $2, $3, $4, false)
                `, [documentoId, curso || null, classe || null, turma !== 'todos' ? turma : null]);
            }
        }
        return documentoId;
    } catch (err) {
