...
exports.update_encarregado = async (req, res) => {
    ...
    if (!telefoneRegex.test(telefone)) {
        return res.json({ sucesso: false, motivo: "Telefone deve conter exatamente 9 dígitos." });
    }
    try {
        const result = await atualizarEncarregado(nome, email, telefone, id);

        if (result.sucesso) {
            return res.json({ sucesso: true, motivo: result.motivo });
        } else {
            return res.json({ sucesso: false, motivo: result.motivo });
        }
    } catch (error) {
        console.error('Erro ao atualizar o encarregado:', error);
        return res.json({ sucesso: false, motivo: 'Erro interno do servidor.' });
    }
};