...
exports.update_encarregado = async (req, res) => {
    try {
        ...
        const existingUser = await User.findOne({
            where: { [Op.or]: [{ email }, { telefone }] }
        });

        if (existingUser) {
            return res.json({ error: "Este email ou telefone já está registrado." });
        }

        // Se não existir, atualiza o usuário
        const updatedUser = await User.update(
            { nome, email, telefone }, // Dados a serem atualizados
            { where: { email } } // Aqui você pode ajustar a busca pelo id ou outro campo
        );

        if (updatedUser) {
            return res.json({ success: true }); // Resposta de sucesso
        } else {
            return res.json({ error: "Falha na atualização." }); // Caso algo tenha dado errado
        }
