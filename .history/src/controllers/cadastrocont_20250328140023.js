const { buscarEncarregadoPorEmail,buscarEncarregadoPorTelefone,buscarEncarregadoPorNome,buscaridestudante,logEvent,buscarToken, adicionar_senha, deletarToken, estudanteExiste, buscarEncarregadoPorEmail, criarEncarregado, gerarTokenRedefinicao, cadastrarEstudanteNoBanco } = require('../db');
...
exports.cadastrar = async (req, res) => {
    ...
    try {
        ...
        const encarregadoEmail = await buscarEncarregadoPorEmail(encarregado_email);
        ...
        // Se existir qualquer um dos dados isoladamente, valida se todos correspondem
        if (encarregadoEmail || encarregadoTelefone || encarregadoNome) {
            if (
                ...
                // Todos dados batem → pode cadastrar
                ...
                return res.redirect('/dashboard-cordenacao');
            } else {
                // Dados conflitantes → bloqueia
                return res.status(400).send('Dados conflitantes: já existe um encarregado com este email, telefone ou nome.');
            }
        } else {
            // Nenhum dado existe → cria novo encarregado
...
exports.buscar = async (req, res) => {
    ...
    try {
        const encarregado = await buscarEncarregadoPorEmail(email);
        if (encarregado) {
            res.json({ nome: encarregado.nome, telefone: encarregado.telefone });
        } else {
            res.status(404).json({ message: 'Encarregado não encontrado.' });
        }
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Erro ao buscar encarregado.' });
    } 
}
