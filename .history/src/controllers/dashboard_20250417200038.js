...
const { log_atividades, buscar_estudantes_associados, gerarTokenRedefinicao, deletarToken, buscar_encarregado_id, eliminar_tudo, verifica_estudante, atualizarEncarregado, atualizar_estudante, lista_coordenacao, lista_encarregados ,lista_estudantes, findEmail, buscar_logs, find_cordenacao } = require('../db');
...
// Função para enviar contratos
exports.enviar_documento = async (req, res) => {
    const dadosContrato = req.body; // Pega os dados enviados no corpo da requisição
    console.log(dadosContrato); // Exibe no console para verificação
    console.log(dadosContrato.tipoDocumento, dadosContrato.titulo, dadosContrato.conteudo);
    await guardar_documento(dadosContrato.tipoDocumento, dadosContrato.titulo, dadosContrato.conteudo);
...
exports.showtelap = async (req, res) => { 
    try {
        ...
        res.render("dashboard/dashboard",{email, nome, id, estudantes, log_atividade});
    } catch (error) {
