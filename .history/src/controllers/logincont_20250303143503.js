...
exports.verifyCode = async (req, res) => {
    ...
    const tempToken = req.cookies?.tempToken; // Pegando dos cookies

    if (!tempToken) {
        return res.status(400).json({ message: "Token não encontrado. Faça login novamente." });
    }
    
    try {
        const decoded = jwt.verify(tempToken, process.env.TOKEN_SECRET); // Decodificando token
        ...
        const tempToken = jwt.sign({ email }, process.env.TOKEN_SECRET, { expiresIn: "5m" });
        res.cookie("tempToken", token, { httpOnly: true, secure: false, maxAge: 5 * 60 * 1000 });
