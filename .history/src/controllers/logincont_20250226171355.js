...
exports.verifyCode = async (req, res) => {
    ...
    if (!codigo ) {
        return res.status(400).json({ message: "Código é obrigatório ou Token expirado ou inválido." });
    }
    ...
    try {
        ...
        console.log(`Verificando código ${codigo} para o email ${email}`);
        const verificationResult = await verifyVerificationCode(email, codigo); 
        console.log("Resultado da verificação:", verificationResult, codigo);
