const { findEmail, atualizar_login_coordenacao, enviar_codigo, buscar_codigoOTP, deleteVerificationCode, saveVerificationCode, verifyVerificationCode} = require('../db');
...
exports.login_cordenacao = async (req, res) => {
    ...
    console.log(id);
    const tempToken = jwt.sign({ id }, SECRET_KEY, { expiresIn: "5m" });
    res.cookie("tempToken", tempToken, { httpOnly: true, secure: true,  maxAge: 5 * 60 * 1000 });
...
exports.verificar_login_cordenacao = async (req,res) => {
   try{ 
    ...
    const codigo_otp = await buscar_codigoOTP (id, email);
    console.log("Código digitado pelo usuário:", codigo);
    console.log("Código OTP recuperado do banco (criptografado):", codigo_otp.codigo_otp);
    
    // Verifique se o código OTP foi encontrado
    ...
    await atualizar_login_coordenacao(id);
    
    const authToken = jwt.sign({ id }, process.env.TOKEN_SECRET, { expiresIn: "30d" });
    res.cookie("token", authToken, { httpOnly: true, secure: true, maxAge: 30 * 24 * 60 * 60 * 1000 });
    res.clearCookie("tempToken");
