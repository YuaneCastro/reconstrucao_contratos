...
exports.resendConfirmationCode = async (req, res) => {
    ...
    if (!email) {
        console.log('Debug: E-mail ausente ou indefinido na sessão.');
        return res.status(400).send('E-mail não encontrado na sessão. Registre-se novamente.');
    }

    console.log('Debug: E-mail recuperado da sessão:', email);

    if (!pendingUsers.has(email)) {
        console.log('Debug: Usuário não encontrado no pendingUsers:', email);
    ...
    console.log('Debug: Novo código de confirmação gerado:', newCode);
    ...
    try {
        ...
        console.log('Debug: Código reenviado com sucesso para:', email);
