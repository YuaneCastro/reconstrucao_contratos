...
//------------------coordenacao-------------------
exports.dashboard_secretaria = async(req,res) =>{
    try {
        ...
        const log_atividade_encarregados = await log_atividades_encarregados(id);
...
exports.dashboard_cordenacao = async(req,res) =>{
    try {
        ...
        const log_atividade_encarregados = await log_atividades_encarregados(id);
        //console.log(documentos)
        res.render("dashboard/dashboard-administracao", { role:user.role, email: user.email, logs, encarregados, estudantes, coordenacao, comunicado, contrato, documentos, log_atividade_encarregados});
    } catch (error) {
        console.error("Erro ao carregar a dashboard:", error);
        return res.redirect("/Telaerro?message=" + encodeURIComponent("Precisa fazer login para acassar esta tela."));
...
exports.redifinir_senha = async(req, res) => {
    ...
    //  https://reconstrucaocontratos-production.up.railway.app/set-password/${token}
...
exports.enviar_documento = async (req, res) => {
    ...
    try{
        await enviar_documento(dadosContrato.tipoDocumento, dadosContrato.titulo, dadosContrato.conteudo,  dadosContrato.especificacoes, dadosContrato.enviarTodos, dadosContrato.dataExpiracao, dadosContrato.id_documento);
        return res.status(200).json({
            sucesso: true,
...
exports.guardar_documentos = async(req, res) =>{
    ...
    //console.log(dadosContrato)
    try{
        await guardar_documento (dadosContrato.tipoDocumento, dadosContrato.titulo, dadosContrato.conteudo,  dadosContrato.especificacoes, dadosContrato.enviarTodos, dadosContrato.dataExpiracao, dadosContrato.id_documento)
...
exports.encarregados_associados = async(req, res) =>{
    const { contratoId } = req.body;
    try{
        const encarregados = await buscar_encarregados_associados_contratos(contratoId);
        ...
        res.status(500).json({
            sucesso: false,
            motivo: "Erro interno ao buscar encarregados."
        });
    }
...
exports.assinar_contrato = async (req, res) => {
    const { documento_id, assinatura_doc_id, encarregado_id, estudante_id } = req.body;
    const formatIP = (ip) => {
    ...
    try{
        ...
        const acao = 'Assinou contrato';
        const detalhes = `O encarregado ${encarregado_id} assinou o contrato ${documento_id}, referente à assinatura ${assinatura_doc_id} do estudante ${estudante_id}.`;
        await logEvent(encarregado_id, estudante_id, acao, detalhes);
