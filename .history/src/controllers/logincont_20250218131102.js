const { findOTPByUserId, deleteOTP } = require('../db'); // Importa funções de banco de dados
const jwt = require('jsonwebtoken'); // Biblioteca para gerar o token
const SECRET_KEY = 'sua_chave_secreta'; // Define a chave secreta para JWT

// Exibe a página de confirmação de login (renderiza o EJS)
exports.showpageconflogin = (req, res) => {
    res.render('confirmlog'); // Renderiza a página de confirmação do código
};

// Função que valida o código de confirmação enviado
exports.confirmCodelogin = async (req, res) => {
    console.log("Recebendo requisição de confirmação:", req.body);
    
    const { code } = req.body; // O código OTP enviado do frontend
    const userId = req.session.tempUserId; // Obtém o ID do usuário da sessão
