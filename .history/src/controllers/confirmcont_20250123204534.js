...
exports.resendConfirmationCode = async (req, res) => {
    ...
    const userData = pendingUsers.get(email);
    const newCode = Math.floor(100000 + Math.random() * 900000).toString(); // Gera um novo código
    userData.confirmationCode = newCode;
    userData.expiration = Date.now() + 40 * 1000; // Código válido por 40 segundos
    ...
    try {
        ...
        return res.status(200).send('Código reenviado. Verifique seu email.');
    } catch (err) {
