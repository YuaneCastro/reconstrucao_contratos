...
exports.verifyCode = async (req, res) => {
    ...
    try {
        const savedCodeHash = await findCode(email);
        if (!savedCodeHash) return res.status(401).json({ message: "Código inválido ou expirado." });
        
        const isCodeValid = bcrypt.compareSync(code, savedCodeHash);
        if (!isCodeValid) return res.status(401).json({ message: "Código inválido ou expirado." });
