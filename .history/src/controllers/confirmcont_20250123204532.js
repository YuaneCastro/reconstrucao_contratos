...
exports.handleConfirmation = async (req, res) => {
    ...
    try {
        ...
        if (!pendingUsers.has(email)) {
            return res.status(400).alert('Código inválido ou expirado.');
        }
        ...
        if (userData.confirmationCode !== confirmationCode) {
            return res.status(400).alert('Código de confirmação incorreto.');
        }
        if (userData.confirmationCode !== confirmationCode) {
        ...
        res.status(200).json({ success: 'Cadastro confirmado com sucesso! Redirecionando...' });
...
exports.resendConfirmationCode = async (req, res) => {
    ...
    const userData = pendingUsers.get(email);
    const newCode = Math.floor(100000 + Math.random() * 900000).toString(); // Código de 6 dígitos como string
    userData.confirmationCode = newCode;
    userData.expiration = Date.now() + 40 * 1000; // 40 segundos de validade
