...
const { findEmail, find_cordenacao, getLogEvents } = require('../db');
...
exports.showtelap = async (req, res) => { 
    try {
        ...
        res.render("dashboard/dashboard", { nome: user.nome, email: user.email, logs });
...
exports.dashboard_cordenacao = async(req,res) =>{
    try {
        ...
        if (!token) {return res.redirect("/login")};
        const decoded = jwt.verify(token, process.env.TOKEN_SECRET);
        const id = decoded.ip_user;
        const user = await find_cordenacao(ip_user); // Verifique se essa função existe

        if (!user) {return res.redirect("/login")}

        const logs = await buscar_logs(user.email, user.ip);
        res.render("dashboard/dashboard-cordenacao", { email: user.email, ip: user.ip, logs});
    } catch (error) {
        console.error("Erro ao carregar a dashboard:", error);
        res.redirect("/login");
    }
