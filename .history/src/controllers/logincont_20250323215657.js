...
exports.verificar_login_cordenacao = async (req,res) => {
   try{ 
    ...
    const codigo_otp = await buscar_codigoUTP(id, email);
    if (!codigo_otp || !codigo_otp.codigo_otp) {
        console.error("❌ Código OTP não encontrado ou inválido.");
        return res.status(400).json({ message: "Código não encontrado ou expirado." });
    }
    console.log("Código OTP encontrado:", codigo_otp.codigo_otp);
    console.log("Código OTP encontrado:", codigo_otp);
    if (typeof codigo !== "string" || codigo.trim() === "") {
        console.error("❌ Código digitado está vazio.");
