...
const { verifica_estudante, verifica_encarregado, atualizar_encarregado, atualizar_estudante, lista_coordenacao, lista_encarregados ,lista_estudantes, findEmail, buscar_logs, find_cordenacao } = require('../db');
...
exports.update_Student = async (req, res) => {
    ...
    try{
        const result = await verifica_estudante(id, nome)
        if (result.rows.length > 0) {
            return res.status(400).json({
                sucesso: true,
                motivo: "Já existe um estudante com este nome."
            });
        }
...
exports.update_encarregado = async (req, res) => {
    ...
    try{
        const result = await verifica_encarregado(nome, email, telefone, id);
        if (result.length > 0) {
            return res.status(400).json({
                sucesso: true,
                motivo: "Já existe um encarregado com este nome, email ou telefone."
            });
        }
        await atualizar_encarregado(id, nome, email, telefone);
        return res.status(200).json({
            sucesso: true,
            motivo: "atualizado com sucesso"
        });
...
}; verifica_estudante, verifica_encarregado