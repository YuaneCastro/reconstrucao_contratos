...
const { lista_coordenacao, lista_encarregados ,lista_estudantes, findEmail, buscar_logs, find_cordenacao } = require('../db');
...
exports.dashboard_cordenacao = async(req,res) =>{
    try {
        ...
        res.render("dashboard/dashboard-cordenacao", { email: user.email, role:user.role, logs, encarregados, estudantes, coordenacao});
    } catch (error) {
        console.error("Erro ao carregar a dashboard:", error);
