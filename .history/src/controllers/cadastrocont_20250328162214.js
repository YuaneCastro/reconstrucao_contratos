const { buscar_email, buscarEncarregadoPorTelefone,buscarEncarregadoPorNome,buscaridestudante,logEvent,buscarToken, adicionar_senha, deletarToken, estudanteExiste, criarEncarregado, gerarTokenRedefinicao, cadastrarEstudanteNoBanco } = require('../db');
...
exports.cadastrar = async (req, res) => {
    ...
    try {
        ...
        if (await estudanteExiste(nome)) {
            return res.status(400).json({ sucesso: false, mensagem: 'Este estudante já foi cadastrado.' });
        }
        ...
        if (encarregadoEmail || encarregadoTelefone || encarregadoNome) {
            if (
                ...
                // Dados conflitantes
                return res.status(409).json({ sucesso: false, mensagem: 'Verifique as informações do encarregado já cadastrado.' });
            }
        } else {
            ...
            return res.json({ sucesso: true, mensagem: 'Estudante cadastrado com sucesso.' });
        }        
    } catch (error) {
        console.error(error);
