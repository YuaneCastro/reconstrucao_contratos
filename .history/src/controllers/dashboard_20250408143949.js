...
exports.update_encarregado = async (req, res) => {
    ...
    try {
        const result = await atualizarEncarregado(nome, email, telefone, id);

        if (result.success) {
            res.json({ sucesso: true });
        } else {
            res.json({ sucesso: false, motivo: result.motivo });
        }
    } catch (error) {
        console.error('Erro ao atualizar o encarregado:', error);
        res.json({ sucesso: false, motivo: 'Erro interno do servidor.' });
    }
};