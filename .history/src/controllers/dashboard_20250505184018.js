...
const { getContratoPorId, buscarContratosPendentes, buscar_documento_contrato, buscar_documento_comunicado, buscarDocumentosDoEncarregado, verificarPermissaoDocumento, buscarDocumentoPorId, guardar_documento_e_enviar, log_atividades, buscar_estudantes_associados, gerarTokenRedefinicao, deletarToken, buscar_encarregado_id, eliminar_tudo, verifica_estudante, atualizarEncarregado, atualizar_estudante, lista_coordenacao, lista_encarregados ,lista_estudantes, findEmail, buscar_logs, find_cordenacao } = require('../db');
...
exports.enviar_documento = async (req, res) => {
    ...
    try{
        await guardar_documento_e_enviar(dadosContrato.tipoDocumento, dadosContrato.titulo, dadosContrato.conteudo,  dadosContrato.especificacoes, dadosContrato.enviarParaTodos, dadosContrato.dataExpiracao);
        return res.status(200).json({
            sucesso: true,
            motivo: "SUCESSO AO CONECTAR COM O METODO"
        });
    }catch(err){
        console.log(err)
        return res.status(500).json({
...
exports.assinar_contrato = async (req, res) => {
    ...
    console.log(assinatura_doc_id, encarregado_id, estudante_id);
     const result = await assinar_contrato(assinatura_doc_id, encarregado_id, estudante_id)
};