...
exports.verifyCode = async (req, res) => {
    const { code } = req.body;
    const email = req.session.email; // Pegando o email da sessão

    console.log("Sessão atual:", req.session); // Para verificar se o email está na sessão
    console.log("Código recebido do usuário:", code);

    if (!email) {
        console.log("Erro: Email não encontrado na sessão.");
        return res.status(401).json({ message: "Acesso negado. Faça login novamente." });
    }

    try {
        // Verificar se o código existe no banco
        const savedCode = await findCode(email, code);
        console.log(`Código salvo no banco para ${email}:`, savedCode);        
        ...
        // Apagar o código do banco após o uso
        await deleteCode(email);
        console.log("Código removido do banco com sucesso.");

        // Criar token de autenticação
        const token = jwt.sign({ userId: user.id }, SECRET_KEY, { expiresIn: "24h" });
        res.cookie("ctoken", token, { httpOnly: true, secure: true, maxAge: 24 * 60 * 60 * 1000 });

        console.log("Login confirmado! Redirecionando...");
