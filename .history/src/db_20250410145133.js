...
const eliminar_tudo = async (id) => {
    ...
    try {
        ...
        // Deleta os logs de atividades relacionados ao encarregado
        await client.query(
            `DELETE FROM log_atividades WHERE encarregado_id = $1`, 
            [id]
        );

        // Deleta os tokens de redefinição de senha relacionados ao encarregado
        await client.query(
            `DELETE FROM tokens_redefinicao WHERE encarregado_id = $1`, 
            [id]
        );

        // Remove os estudantes que pertencem ao encarregado
        await client.query(
            `DELETE FROM estudantes WHERE encarregado_id = $1`, 
            [id]
        );

        // Por fim, remove o próprio encarregado
        const result = await client.query(
            `DELETE FROM encarregados WHERE id = $1`, 
            [id]
        );
        ...
        await client.query('ROLLBACK');
        console.error("Erro ao deletar o encarregado e registros relacionados:", erro);
        return {
            sucesso: false,
