...
const enviar_codigo = async (email, bcrypt_code, ip_user) => {
    try{
        ...
        `INSERT INTO logins_coordenacao (email, codigo_otp, ip, usado, data_envio, data_login) 
         VALUES( $1, $2, $3, $4, $5, $6)
         RETURNING id`,
         [email, bcrypt_code, ip_user, FALSE, new Date(), NULL]
    );
...
const buscar_codigoUTP = async(ip_user, email) => {
    const result = await pool.query(
        `SELECT id, codigo_otp FROM logins_coordenacao WHERE ip = $1 AND email = $2`,
        [ip_user, email]);
        return result.rows.length > 0 ? result.rows[0] : null;
};
const atualizar_login_cordenacao = async(id) =>{
    await pool.query("UPDATE logins_coordenacao SET usado = true, data_login = NOW() WHERE id = $1",
    );
};
