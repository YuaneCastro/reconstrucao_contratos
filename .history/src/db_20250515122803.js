...
const guardar_documento = async(id, tipo, titulo, descricao = null, especificacoes = [],  enviarParaTodos = false, dataExpiracao = null, id=null) => {
    try {
        let documentoId = id;

        if (!documentoId) {
            const dataExpiracaoFinal = dataExpiracao ? dataExpiracao : null;
            const result = await pool.query(`
                INSERT INTO documentos (tipo, titulo, descricao, data_expiracao, data_emissao)
                VALUES ($1, $2, $3, $4, NULL)
                RETURNING id
            `, [tipo, titulo, descricao, dataExpiracaoFinal]);
...
const buscar_documentos_nao_emitidos = async () =>{
    try{
        const result = await pool.query(`SELECT * FROM documentos WHERE data_emissao IS NULL ORDER BY id DESC`);
        return result.rows;
    }catch(err){
        console.log(err);
        throw err;
