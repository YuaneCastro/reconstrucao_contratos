...
const atualizar_encarregado = async (nome, email, telefone, id) => {
    try{
        await query.pool(`UPDATE encarregados SET nome = $1, email = $2, telefone = $3 WHERE id = $4`,
...
const atualizar_estudante = async (id, nome, classe, turma, curso) => {
    try{
        await query.pool(`UPDATE estudantes SET nome = $1, classe = $2, turma = $3, curso = $4 WHERE id = $5`,
        [nome, classe, turma, curso || null, id]);
    }catch(error){
        console.error("Erro ao atualizar estudante:", err);
    }
}
const verifica_estudante = async() => {
    await query.pool(`SELECT * FROM estudantes WHERE nome = $1 AND id != $2`)
}
const verifica_encarregado = async() => {
    await query.pool(`SELECT * FROM encarregados WHERE (nome = $1 OR email = $2 OR telefone = $3) AND id != $4`,
    [nome, email, telefone, id]);
