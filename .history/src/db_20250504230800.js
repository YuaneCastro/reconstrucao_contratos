...
const buscarDocumentosDoEncarregado = async (id) => {
    const result = await pool.query(`
        SELECT d.id, d.tipo, d.titulo, d.descricao, d.data_emissao, d.data_expiracao, e.nome AS estudante_nome 
        FROM documentos d
...
async function buscarContratosPendentes(id) {
    const query = `
      SELECT d.id AS documento_id, d.tipo, d.titulo, d.descricao, d.data_emissao, d.data_expiracao, ad.estudante_id, e.nome AS estudante_nome
      FROM documentos d
