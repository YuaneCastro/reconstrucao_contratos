...
const eliminar_tudo = async (id) => {
    const client = await pool.connect(); // Obtém o cliente
    try {
        await client.query('BEGIN'); // Inicia a transação

        // Deleta os dados nas diferentes tabelas
        await client.query('DELETE FROM log_atividades WHERE encarregado_id = $1', [id]);
        await client.query('DELETE FROM estudantes WHERE encarregado_id = $1', [id]);
        await client.query('DELETE FROM tokens_redefinicao WHERE encarregado_id = $1', [id]);
        await client.query('DELETE FROM encarregados WHERE id = $1', [id]);
        ...
        await client.query('ROLLBACK'); // Se houver erro, desfaz tudo
    } finally {
